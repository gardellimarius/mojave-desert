...
/******/ 	// eslint-disable-next-line no-unused-vars
/******/ 	var hotCurrentHash = "632a9317415d153db544";
...
var scss_style = __webpack_require__(9);
...
_lastMediaTick,
  ...
  if (_lastMediaTick !== tick) {
    ...
    for (; i < _media.length; i += 2) {
      ScrollTrigger_win.matchMedia(_media[i]).matches ? matches.push(i) : _revertAll(1, _media[i]); // Firefox doesn't update the "matches" property of the MediaQueryList object correctly - it only does so as it calls its change handler - so we must re-create a media query here to ensure it's accurate.
    }
...
ScrollTrigger.matchMedia({
  ...
  "all": function() {
    ...
    // so they persist.
    let speed = 100;
    let scene1 = gsapWithCSS.timeline({defaults: {ease: "power1.in"}});
    ...
    gsapWithCSS.utils.toArray(".move").forEach(elem => {
      scene1.to(elem, {y: elem.dataset.speed * speed}, 0)
    });


    gsapWithCSS.utils.toArray(".fade").forEach(elem => {
    ...
    gsapWithCSS.utils.toArray(".fade-mobile").forEach(elem => {
      gsapWithCSS.from(elem, {
        autoAlpha: 0,
        y: elem.dataset.speed * speed,
        scrollTrigger: {
    ...
    gsapWithCSS.utils.toArray(".line").forEach(elem => {
      gsapWithCSS.from(elem, {
        scaleY: 0,
    ...
    gsapWithCSS.utils.toArray(".fade-fromMargin").forEach(elem => {
      gsapWithCSS.from(elem, {
        autoAlpha: 0,
    ...
    gsapWithCSS.utils.toArray(".fade-fromMarginLeft").forEach(elem => {
      gsapWithCSS.from(elem, {
        autoAlpha: 0,
        //y: elem.dataset.speed * speed,
        x: - elem.dataset.speed * speedfadefromMargin,
    ...
    gsapWithCSS.utils.toArray(".fade-fromMarginRight").forEach(elem => {
      gsapWithCSS.from(elem, {
        autoAlpha: 0,
        //y: elem.dataset.speed * speed,
        x: elem.dataset.speed * speedfadefromMargin,
    ...
    gsapWithCSS.utils.toArray(".fade-no-scrub").forEach(elem => {
      gsapWithCSS.from(elem, {
        autoAlpha: 0,
        y: elem.dataset.speed * speed,
        scrollTrigger: {
